...
4. **Demo UserOp** → Gas-free 0.01 ETH transfer via custom paymaster
...
Gas-free UserOps via `MyPaymaster.sol` on Abstract testnet

```
┌──────────┐     ┌──────────────┐     ┌──────────────┐
│  Browser  │     │  Paymaster   │     │  Bootloader  │
│  (viem)   │     │  0xEdfC...   │     │              │
├──────────┤     ├──────────────┤     ├──────────────┤
│ UserOp   │────►│ validateAnd  │────►│ Pay gas      │
│ w/       │     │ PayForPaym.  │     │ (general     │
│ paymaster│     │              │     │  flow)       │
│ input    │     │              │     │              │
└──────────┘     └──────────────┘     └──────────────┘
```

- Supports `general(bytes)` flow only (not `approvalBased`)
- Deployed at `0xEdfC685a0D638820A50471d7a7c7061B8fFA912D`
- Funded with 0.02 ETH, verified on explorer
- Users pay zero gas — paymaster sponsors all fees
...
| 11 | Paymaster: custom MyPaymaster.sol, gas-free UserOps |
...
**AI-Enable** — built entirely with opencode.

No manual code written. All features implemented through natural language chat.
